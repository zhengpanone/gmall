...
(
    ...
    deleted      bit          DEFAULT b'0'              NOT NULL comment '是否删除',
    deleted_time datetime     DEFAULT NULL              NULL comment '删除时间',
    UNIQUE KEY `uk_dict_type_id_code` (`type_id`, `data_code`) USING BTREE,
    UNIQUE KEY `uk_dict_type_code_code` (`type_code`, `data_code`) USING BTREE,
    KEY `idx_dict_id` (`type_id`) USING BTREE,
    KEY `idx_dict_code` (`type_code`) USING BTREE,
    KEY `idx_data_code` (`data_code`) USING BTREE,
    KEY `idx_status` (`status`) USING BTREE,
    KEY `idx_deleted` (`deleted`) USING BTREE,

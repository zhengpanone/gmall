package com.zp.gmall.framework.tenant.core.security;
...
import com.zp.gmall.framework.tenant.config.TenantProperties;
...
import com.zp.gmall.framework.web.core.filter.ApiRequestFilter;
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        ...
        if (Objects.nonNull(loginUser)) {
            ...
            if (StrUtil.isBlank(tenantId)) {
                ...
                ServletUtils.writeJSON(response, Result.failed(GlobalErrorCodeConstants.FORBIDDEN.getCode(), "您无权访问该租户的数据"));
                return;
            }
        }
        ...
        if (!isIgnoreUrl(request)) {
            ...
            if (tenantId == null) {
                ...
                ServletUtils.writeJSON(response, Result.failed(GlobalErrorCodeConstants.BAD_REQUEST.getCode(), "请求的租户标识未传递，请进行排查"));
                return;
            }
            // 3. 校验租户是合法，例如说被禁用、到期
            try {

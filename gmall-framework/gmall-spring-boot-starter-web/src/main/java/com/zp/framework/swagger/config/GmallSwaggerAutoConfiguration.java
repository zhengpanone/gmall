package com.zp.framework.swagger.config;
...
public class GmallSwaggerAutoConfiguration {
    ...
    /**
     * 安全模式，这里配置通过请求头 Authorization传递token参数
     */
    private Map<String, SecurityScheme> buildSecuritySchemes() {
        HashMap<String, SecurityScheme> securitySchemeMap = new HashMap<>();
        SecurityScheme securityScheme = new SecurityScheme()
                .type(SecurityScheme.Type.APIKEY) // 类型
                .name(HttpHeaders.AUTHORIZATION) // 请求头的name
                .in(SecurityScheme.In.HEADER); // token所在位置
        securitySchemeMap.put(HttpHeaders.AUTHORIZATION, securityScheme);
    ...
    public OpenAPIService openApiBuilder(Optional<OpenAPI> openAPI,
                                         ...
                                         Optional<List<OpenApiBuilderCustomizer>> openApiCustomizer,
                                         Optional<List<ServerBaseUrlCustomizer>> serverCustomizer,
                                         Optional<JavadocProvider> javadocProvider
    ) {
        return new OpenAPIService(openAPI, securityService, springDocConfigProperties, propertyResolverUtils, openApiCustomizer, serverCustomizer, javadocProvider);
